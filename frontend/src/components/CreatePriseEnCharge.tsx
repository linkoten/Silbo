...
import { priseEnChargeFormSchema } from "./userFormSchema"; // Ou le chemin correct
...
const CreatePriseEnChargePage: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const dataToSend = {
      ...
      datePriseEnCharge:
        formData.datePriseEnCharge instanceof Date
          ? formData.datePriseEnCharge.toISOString()
          : new Date(formData.datePriseEnCharge).toISOString(),
    };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.details || "Erreur lors de la création du priseEnCharge"
        );
      }

      // Redirection vers la liste des prisesEnCharge après création réussie
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-6">
        Ajouter un nouveau priseEnCharge
      </h1>
      ...
      >
        ...
        <div className="mb-4">
          <label
            className="block text-gray-700 text-sm font-bold mb-2"
            htmlFor="datePriseEnCharge"
          >
            Date de Prise En Charge*
          </label>
          <input
            className={`shadow appearance-none border ${
              errors.datePriseEnCharge ? "border-red-500" : ""
            } rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`}
            ...
            value={
              formData.datePriseEnCharge instanceof Date
                ? formData.datePriseEnCharge.toISOString().split("T")[0]
                : new Date(formData.datePriseEnCharge)
                    .toISOString()
                    .split("T")[0]
            }
            onChange={handleChange}
